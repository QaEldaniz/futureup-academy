...
export default function StudentGradesPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('myGrades')}</h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">{t('overallAverage')}</p>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">{t('totalGrades')}</p>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          ...
          {['ASSIGNMENT', 'QUIZ', 'EXAM', 'PROJECT', 'PARTICIPATION'].map((tp) => (
            <option key={tp} value={tp}>{tp.charAt(0) + tp.slice(1).toLowerCase()}</option>
          ))}
        </select>
      </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{tField(g.course, 'title')}</p>
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_COLORS[g.type] || ''}`}>
                        {g.type}
                      </span>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                      {g.lesson && <span>{tField(g.lesson, 'title')}</span>}
                      <span>{t('by')} {tField(g.teacher, 'name')}</span>
