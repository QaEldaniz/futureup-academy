...
export default function TeacherStudentsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('allStudents')}</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-12 text-center">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white">{t('noStudentsFound')}</h3>
        </div>
      ) : (
          ...
          {filtered.map((s: any) => (
            <div key={s.id} className="flex items-center gap-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-bold text-sm">
                {(tField(s, 'name') || 'S').charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 dark:text-white text-sm truncate">{tField(s, 'name')}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{s.email}</p>
              </div>
              <span className="text-xs text-gray-400">{s.courses?.length || 0} {t('courses')}</span>
            </div>
          ))}
        </div>
      )}
