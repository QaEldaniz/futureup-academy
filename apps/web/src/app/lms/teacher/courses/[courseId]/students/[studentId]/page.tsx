    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">{data?.student?.name}</h1>
      ...
      {data?.lessons && (
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-3">Lesson Progress</h2>
            ...
            {data.lessons.map((l: any, i: number) => {
              ...
              return (
                <div key={l.id} className="flex items-center gap-3 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-3">
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-white">{i + 1}. {l.titleEn}</span>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
        ...
        <div className="flex gap-2 mb-3">
          {['PROGRESS', 'BEHAVIOR', 'ACHIEVEMENT', 'GENERAL'].map((t) => (
            <button
              key={t}
              onClick={() => setCommentType(t)}
              className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                commentType === t ? 'bg-primary-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'
              }`}
            >
