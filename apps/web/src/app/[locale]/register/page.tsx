...
import { Eye, EyeOff, UserPlus, GraduationCap, ArrowLeft, Mail, RefreshCw } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.data?.requiresVerification || data.message === 'REGISTRATION_PENDING') {
        setStep('verify');
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.message === 'EMAIL_VERIFIED_PENDING_APPROVAL') {
        setSuccess(t('verifiedPendingApproval'));
        return;
      }
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl shadow-gray-200/50 dark:shadow-black/20 border border-gray-100 dark:border-gray-800 p-8">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-primary-500 to-secondary-600 mb-4">
              {step === 'verify' ? <Mail className="w-8 h-8 text-white" /> : <GraduationCap className="w-8 h-8 text-white" />}
            </div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              {step === 'verify' ? t('verifyTitle') : t('title')}
            </h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
              {step === 'verify' ? t('verifySubtitle') : t('subtitle')}
            </p>
            {step === 'verify' && (
