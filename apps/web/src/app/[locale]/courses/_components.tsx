...
import {
  ...
  Loader2, Gamepad2, Brain, Baby, Users,
} from 'lucide-react';
...
export function getLocalized(item: Record<string, unknown>, field: string, locale: string): string {
...
export function CourseCard({ course, locale, isKids }: { course: Course; locale: string; isKids: boolean }) {
...
export default function CoursesPageContent({ audience }: CoursesPageContentProps) {
  ...
  const handleCategoryChange = (cat: string) => {
    ...
    updateURL(cat, selectedAgeGroup, searchQuery);
  };

  const handleAgeChange = (age: string) => {
    setSelectedAgeGroup(age);
    updateURL(selectedCategory, age, searchQuery);
  };

  const handleSearchChange = (q: string) => {
    setSearchQuery(q);
    updateURL(selectedCategory, selectedAgeGroup, q);
  };
  ...
  }, [courses, selectedCategory, audience, selectedAgeGroup, searchQuery, locale]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-col gap-4 mb-10">
            <div className="relative w-full">
              ...
              <input type="text" placeholder={t('search') || 'Search...'} value={searchQuery} onChange={(e) => handleSearchChange(e.target.value)}
                className={cn(
            ...
            {isKids && (
              <div className="flex flex-wrap gap-2">
                <button onClick={() => handleAgeChange('all')}
          ...
          {loading ? (
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="visible" key={selectedCategory + searchQuery + selectedAgeGroup} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
